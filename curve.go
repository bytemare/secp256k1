...
func map2IsoCurve(fe *big.Int) *Element {
	x, y := mapToCurveSSWU(&fp, secp256k13ISOA, secp256k13ISOB, mapZ, fe)
...
func mapToCurveSSWU(f *field.Field, a, b, z, fe *big.Int) (x, y *big.Int) {
	...
	f.Mul(y, &tv1, fe)                              //    19.   y = tv1 * u
	f.Mul(y, y, &_y1)                               //    20.   y = y * y1
	f.CondMov(x, x, &tv3, isGx1Square)              //    21.   x = CMOV(x, tv3, isGx1Square)
	f.CondMov(y, y, &_y1, isGx1Square)              //    22.   y = CMOV(y, y1, isGx1Square)
	e1 := f.Sgn0(fe) == f.Sgn0(y)                   //    23.  e1 = sgn0(u) == sgn0(y)
	f.CondMov(y, f.Neg(&big.Int{}, y), y, e1)       //    24.   y = CMOV(-y, y, e1)
	f.Inv(&tv4, &tv4)                               //    25.   1 / tv4
	f.Mul(x, x, &tv4)                               //	 26.   x = x / tv4
